...
function deleteArticle(url, request) {
  ...
  if (savedArticle) {
    ...
    // deletes all comments under article
...
If body isn’t supplied, user with supplied username doesn’t exist, or article 
with supplied article ID doesn’t exist, returns a 400 response
*/
...
If no ID is supplied or comment with supplied ID doesn’t exist, returns 404 
response
*/
function deleteComment(url,request) {
  ...
  if (savedComment) {
    ...
    // array of all comment IDs under a specific user
    const userCommentIds = database.users[savedComment.username].commentIds;

    // array of all comment IDs under a specific article
    const articleCommentIds = database.articles[savedComment.articleId].commentIds;

    // remove comment (element) from database under the user and article arrays 
    // the comment is attached to and merge the array over the now empty index
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
...
/*
PUT
Receives comment ID from URL parameter and username from username property of 
request body
Adds supplied username to upvotedBy of corresponding comment if user hasn’t 
already upvoted the comment, removes username from downvotedBy if that user 
had previously downvoted the comment, returns 200 response with comment on 
...
already downvoted the comment, remove username from upvotedBy if that user had 
previously upvoted the comment, returns 200 response with comment on comment 
